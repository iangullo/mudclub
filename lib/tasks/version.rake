def valid? version
  pattern = /^\d+\.\d+\.*\d*(\-(dev|alpha|beta|rc\d+))?$/
...
-----------------------
rake version:read							# read latest stored version
...
namespace :version do
  ...
  desc "Read and output current build version"
  task :build, [:version] do |task, args|
		gitver   = `git rev-parse --short HEAD`.strip
    puts "#{gitver}"
  end
  
  desc "Read and output current version"
  task :read, [:version] do |task, args|
  ...
  desc "Sets the alpha flag on"
  task :alpha do
    version, flag = read_version.split '-'
    new_version = [version, 'alpha'].join '-'
    write_version new_version
    puts "Version in alpha: #{new_version}"
  end
