...
class DropdownComponent < ApplicationComponent
	...
	def parse(button)
		...
		case @button[:kind]
		when "add","link"
			@button[:d_class] = "hidden rounded-md bg-gray-100 text-gray-500 text-left font-semibold overflow-hidden no-underline"
	...
	def set_bclass
		...
		when "menu"
			@button[:b_class] = @button[:class] ? @button[:class] : "rounded-md hover:bg-blue-700 hover:text-white focus:bg-blue-700 focus:text-white focus:ring-2 focus:ring-gray-200 whitespace-nowrap shadow rounded ml-2 px-2 py-2 rounded-md font-semibold"
		end
	end
end
