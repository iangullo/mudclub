...
class Drill < ApplicationRecord
	...
	scope :by_name, -> (name) { (name.present? ? search_by_name(name) : all).distinct }
	scope :by_kind, -> (kind_id) { (kind_id.to_i > 0) ? where(kind_id: kind_id.to_i) : where("kind_id>0") }
	scope :by_skill, -> (skill_id) { (skill_id.to_i > 0) ? joins(:skills).where(skills: {id: skill_id.to_i}) : all	}
