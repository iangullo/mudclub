...
class User < ApplicationRecord
  ...
  def is_athlete?
    self.player? || self.athlete.present?
  end

  alias is_player? is_athlete?
  ...
  def team_list
    c_teams = self.is_coach? ? self.coach.teams : []
    j_teams = self.is_player? ? self.player.teams : []
    (c_teams + j_teams).uniq.sort_by { |team| team.season.start_date }.reverse
  end
