...
module DivisionsHelper
	...
	def division_grid
		...
		title << button_field({kind: "add", url: new_sport_division_path(sport_id: @sport.id), frame: "modal"}) if u_admin? || u_manager?

		rows = Array.new
		@divisions.each { |div|
			row = {url: edit_sport_division_path(id: div.id, sport_id: div.sport_id), frame: "modal", items: []}
			row[:items] << {kind: "normal", value: div.name}
			row[:items] << button_field({kind: "delete", url: sport_division_path(id: div.id, sport_id: div.sport_id), name: div.name}) if u_admin? || u_manager?
			rows << row
		}
