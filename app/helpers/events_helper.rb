module EventsHelper
	...
	def event_attendance_title(event:)
	...
	def event_task_title(event:, subtitle:)
		res = event_title_fields(event:, subtitle:, cols: 3)
	end
	...
	# FieldCompoents to show an attendance form
	...
	def match_show_fields(event:)
		...
		res << [{kind: "gap", size: 2}, {kind: "grid", value: period_grid(event:, periods: event.periods), cols: 3}]
	end
	...
	def match_form_fields(event:)
		...
		if periods
			grid = period_grid(event:, periods: periods, edit: true)
		else
	...
	private
		...
		def rest_title(event:, team: nil, season: nil, res:, cols:, form:)
		...
		def match_title(event:, res:, cols:, form:)
			if form
