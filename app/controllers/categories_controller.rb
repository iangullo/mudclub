class CategoriesController < ApplicationController
  ...
  def create
    if current_user.present? and current_user.admin?
      ...
      respond_to do |format|
        if @category.save
          format.html { redirect_to categories_url, notice: "#{I18n.t(:cat_created)} '#{@category.name}'" }
    ...
    if current_user.present? and current_user.admin?
      respond_to do |format|
        if @category.update(category_params)
          format.html { redirect_to categories_url, notice: "#{I18n.t(:cat_updated)} '#{@category.name}'" }
  ...
  def destroy
    if current_user.present? and current_user.admin?
      ...
      @category.destroy
      respond_to do |format|
        format.html { redirect_to categories_url, notice: "#{I18n.t(:cat_deleted)} '#{c_name}'" }
