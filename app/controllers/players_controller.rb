class PlayersController < ApplicationController
	...
	def index
		if current_user.present? and (current_user.admin? or current_user.is_coach?)
			...
			@title << [{kind: "search-text", key: :search, value: session.dig('player_filters', 'search'), url: players_path}]
			...
			respond_to do |format|
				...
				if @player.is_duplicate? then
					...
					if @player.save
						...
						format.html { redirect_to players_url(search: @player.person.name), notice: t(:player_created) + "'#{@player.to_s}'" }
		...
		if current_user.present? and (current_user.admin? or current_user.is_coach? or current_user.person.player_id==@player.id)
			respond_to do |format|
				if @player.update(player_params)
					format.html { redirect_to players_url(search: @player.person.name), notice: t(:player_updated) + "'#{@player.to_s}'" }
