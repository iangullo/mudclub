class DivisionsController < ApplicationController
  ...
  def create
    if current_user.present? and current_user.admin?
      ...
      respond_to do |format|
        if @division.save
          format.html { redirect_to divisions_url, notice: "#{I18n.t(:div_created)} '#{@division.name}'" }
    ...
    if current_user.present? and current_user.admin?
      respond_to do |format|
        if @division.update(division_params)
          format.html { redirect_to divisions_url, notice: "#{I18n.t(:div_updated)} '#{@division.name}'" }
          format.json { render :index, status: :created, location: divisions_url }
        else
  ...
  def destroy
    if current_user.present? and current_user.admin?
      ...
      @division.destroy
      respond_to do |format|
        format.html { redirect_to divisions_url, notice: "#{I18n.t(:div_deleted)} '#{d_name}'" }
