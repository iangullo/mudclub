class SeasonsController < ApplicationController
    ...
    if current_user.present? and current_user.admin?
      ...
      @eligible_locations = @season.eligible_locations
	    ...
	    respond_to do |format|
	      if @season.save
	        format.html { redirect_to seasons_path(@season), notice: "#{I18n.t(:sea_created)} '#{@season.name}'" }
  ...
  def update
        ...
        check_locations
      	if @season.update(season_params)
	        format.html { redirect_to seasons_path(@season), notice: "#{I18n.t(:sea_updated)} '#{@season.name}'" }
					format.json { render :index, status: :created, location: seasons_url, notice: "#{I18n.t(:sea_updated)} '#{@season.name}'" }
	      else
  ...
  def destroy
      ...
      s_name = @season.name
			...
			@season.destroy
	    respond_to do |format|
	      format.html { redirect_to seasons_path, notice: "#{I18n.t(:sea_deleted)} '#{s_name}'" }
